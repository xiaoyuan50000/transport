...
const InitFilter = async function () {
    ...
    $("#indent-vehicle-type").append(DOMPurify.sanitize(`<option value="">Resource: All</option>`))
    for (let item of vehicleTypeSelect) {
        $("#indent-vehicle-type").append(DOMPurify.sanitize(`<option value="${item.typeOfVehicle}">${item.typeOfVehicle}</option>`))
    }
}
...
const InitUnitSearch = function () {
    ...
    const UnitOnFocus = function (e) {
        ...
        for (let item of unitDatas) {
            $(e).next().find(".form-search-select").append(DOMPurify.sanitize(`<li data-id="${item.id}">${item.groupName}</li>`))
        }
    }
    ...
    const InsertFilterOption = function (element, filterUnits) {
        ...
        for (let item of filterUnits) {
            $(element).next().append(DOMPurify.sanitize(`<li data-id="${item.id}">${item.groupName}</li>`))
        }
    }
}