...
const InitDestinations = async function () {
    ...
    const DestinationOnFocus = function (e) {
        ...
        $(e).next().find(".form-search-select").append(DOMPurify.sanitize(destinations.map(item => `<li>${item}</li>`).join('')))

    }

    const InsertFilterOption = function (element, filterDestination) {
        $(element).next().empty()
        $(element).next().append(DOMPurify.sanitize(filterDestination.map(item => `<li>${item}</li>`).join('')))
    }
}
...
const GetContractPartNoByContractNo = async function (val) {
    return await axios.post("/getUnassignedContractPartNo", { contractNo: val }).then(res => {
        ...
        if (length > 0) {
            ...
            $(contractPartNoElem).append(DOMPurify.sanitize(data));
        }
        return length
    })
}
